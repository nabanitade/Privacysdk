...
import { ScannedFile } from "../../scanners/Scanner";
...
export class GeminiPrivacyRule implements Rule {
    id = "GEMINI001";
    description = "Google Gemini 3 Pro Preview (Vertex AI) Privacy Scanning";
    private config: GeminiConfig;
    private ai?: GoogleGenAI;
    private useVertexAI: boolean = true; // Default to Vertex AI
    private apiKey?: string;
    private vertexConfig?: { project: string; location: string };
...
.`;
    ...
    async analyzeCrossFile(files: ScannedFile[]): Promise<Violation[]> {
        const violations: Violation[] = [];
        if (!this.config.enabled) {
            return violations;
        }
    ...
    private buildCrossFilePrompt(files: ScannedFile[]): string {
...
`.trim();
    ...
    private parseCrossFileResponse(response: string): Violation[] {
        const violations: Violation[] = [];
        try {
            const jsonMatch = response.match(/\[[\s\S]*\]/);
            if (!jsonMatch) {
